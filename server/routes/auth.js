...
const { OAuth2Client } = require('google-auth-library'); // <<< 1. IMPORT GOOGLE AUTH LIBRARY
...
// Instantiate Google OAuth Client with credentials from .env file
...
router.post('/google-login', 
  ...
  async (req, res, next) => {
    ...
    try {
        ...
        if (user) {
            // User exists, log them in. If they were inactive, activate them.
            if (!user.isActive) {
                user.isActive = true;
                await user.save();
            }
            ...
            // We must generate a secure, random password for the new user,
            // as our schema requires a password. They will never use it.
            const randomPassword = crypto.randomBytes(32).toString('hex');
            user = new User({
                email,
                password: randomPassword,
                isActive: true // Email is verified by Google, so we can set it as active.
            });
            await user.save();
        }

        // Generate our application's own JWT to manage the session
...
// --- EXISTING ROUTES (No changes needed below this line) ---
...
router.post('/activate-account', [
    ...
    try {
        ...
        if (!user) {
            return res.status(400).json({ error: 'Activation token is invalid or has expired.' });
        }
...
router.post('/request-reset', [
    ...
    try {
        ...
        if (!user || !user.isActive) {
            return res.status(404).json({ error: 'If an account with that email exists and is active, a reset link will be sent.' });
        }
