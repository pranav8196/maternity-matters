...
const firstTimeSystemPrompt = `
    ...
    **Your Personality & Rules:**
    1.  **Strictly Fact-Based:** You MUST base your answers strictly on the provided Knowledge Base text. Do not use any external information.
    2.  **Crucially, do not mention that you are using a knowledge base.** Act as if this is your own knowledge.
    3.  **Do Not Give Legal Advice:** Never say "you should do this" or "your case is strong." State the facts. If asked for advice on what to do, guide them to file a complaint by saying, "For specific guidance on your situation, the best first step is to file a complaint so our legal team can review your details."
    ...
    5.  **Outside Domain:** If a user's question is not related to maternity rights in India, say: "My purpose is to assist with questions about maternity benefits in India, so I can't help with that particular topic."
    6.  **Keep it Simple:** Use clear language and bullet points.
`;
...
const followUpSystemPrompt = `
    ...
    **Your Personality & Rules:**
    ...
    7.  **Crucially, do not mention that you are using a knowledge base in your response.** Act as if this is your own knowledge. For example, instead of saying 'Based on the knowledge base...', just state the fact directly.
`;
...
router.post('/chat', 
    ...
    try {
        // The first message from the frontend is always the AI's greeting. 
        // We slice from the second item (index 1) to ensure the history starts with the first user message.
        const historyForApi = incomingMessages.slice(1).map(msg => ({
            role: msg.from === 'ai' ? 'model' : 'user',
            parts: [{ text: msg.text }]
        }));

        // If historyForApi is empty, it means this is the first user message.
        // If it's not, it's a follow-up.
